...
<body>
    <div>
        ...
        <div class="container mx-auto flex flex-wrap justify-evenly max-w-6xl w-full p-8 mt-2">
            @forelse ($services as $service)
                <div class="flex flex-col items-center p-4 w-1/4 bg-slate-50 rounded-lg m-2 text-wrap">
                    <img class="rounded-lg" src="{{ Storage::url($service->image) }}" alt="Service Image">
                    <h2 class="text-2xl mt-2 font-bold text-center">{{ $service->name }}</h2>
                    <p class="font-semibold text-center">{{ $service->description }}</p>
                </div>
            @empty
                <h1>No services to show</h1>
            @endforelse
        </div>
